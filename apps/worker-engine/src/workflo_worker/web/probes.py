...
def run_web_probes(
    ...
    if _launch_override is not None:
        ...
        try:
            resp = page.goto(base_url, timeout=timeout_ms)
            ...
            results.append({
                "name": "page_loads",
                "passed": False,
                "detail": f"page.goto failed: {e}",
            })

        # Probe 2: no_console_errors — independent of whether the page loaded.
        results.append({
            "name": "no_console_errors",
            "passed": len(console_errors) == 0,
            "detail": console_errors if console_errors else "clean",
