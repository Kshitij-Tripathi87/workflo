# Workflo Platform — Architecture Overview
...
1. **Auth**: Developer runs `workflo auth login` → API key stored in `~/.workflo/config.yaml`
2. **Submit**: `workflo run --security --cloud` → CLI POSTs `RunSpec` to `/v1/runs`
...
All services depend on `packages/workflo-schema` which defines:
- `RunSpec` (CLI → Worker)
...
|-------|----------------|--------------|----------|
| **SaaS** | Workflo Cloud | Direct | Standard users |
| **Enterprise** | Customer's Cloud (VPC) | Outbound to Control Plane | Data residency, low latency |

## Built on top of the `workflo` testing framework

The `workflo` package provides the pytest plugin, isolation verifier, and SOC2 report generator. The Worker Engine imports and wraps these components for cloud execution.
